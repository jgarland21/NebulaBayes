...
class Plot_Config(object):
    ...
    default_cmap = LinearSegmentedColormap.from_list(
        "NB_default", [(0, 0, 0), (56.0 / 255, 132.0 / 255, 0), (1, 1, 1)]
    )
...
def _make_plot_annotation(Plot_Config_1, NB_nd_pdf):
    ...
    make_anno = (Plot_Config_1[pdf_name]["table_on_plot"] is True) and hasattr(
        NB_nd_pdf, "best_model"
    )
    if not make_anno:
    ...
    plot_anno = (
        "Observed fluxes vs. model fluxes at the gridpoint\n"
        "defined by peaks of the 1D marginalised {0} PDFs\n".format(pdf_name.lower())
    )
...
class ND_PDF_Plotter(object):
    ...
    def _prepare_fig_and_axes(self, n):
        ...
        fig, axes = plt.subplots(n, n, figsize=fig_width_ht, gridspec_kw=layout_spec)

        # Add some more information to the layout_spec dict
        layout_spec["axes_width"] = (
            layout_spec["right"] - layout_spec["left"] - (n - 1) * layout_spec["wspace"]
        ) / n  # Fig fraction
        layout_spec["axes_height"] = (
            layout_spec["top"] - layout_spec["bottom"] - (n - 1) * layout_spec["hspace"]
        ) / n  # Fig fraction
        layout_spec["n"] = n
    ...
    def _format_2D_PDF_axes(self, ax, extent):
        ...
        ax.tick_params(direction="out", length=(0.7 * self.tick_size), which="minor")
    ...
    def _add_legend(self, ax_1D, ax_2D, layout_spec, fontsize):
        ...
        legend_anchor = (  # (x,y) in figure fraction coords
            (
                spec["left"]
                + ((n + 1) // 2) * (spec["axes_width"] + spec["wspace"])
                + 0.005
            ),
            (spec["bottom"] + (n // 2) * (spec["axes_height"] + spec["hspace"]) + 0.00),
        )
    ...
    def __call__(self, NB_nd_pdf, out_filename, config=Plot_Config_Default):
        ...
        if out_filename.endswith(".pdf"):  # Add metadata if output is a .pdf
            ...
            metadata_dict["Creator"] = metadata_dict[
                "Creator"
            ] + " ; " + "NebulaBayes {0} ; matplotlib {1} ; numpy {2} ; " "python {3}".format(
                __NB_version__, __mpl_version__, np.__version__, sys.version
            )
    ...
    def interactive(self, NB_nd_pdf, config=Plot_Config_Default):
        ...
        if hasattr(fig, "show") and "inline" not in mpl.get_backend():
            ...
            print(
                "Note: NebulaBayes didn't call 'show' on the current figure "
    ...
    def _plot(self, NB_nd_pdf, config):
        ...
        self._fig.canvas.manager.set_window_title(NB_nd_pdf.name)
        ...
        for double_name, param_inds_double in zip(G.double_names, G.double_indices):
            ...
            extent["xmin"] -= interp_spacing[name_x] / 2.0
            extent["xmax"] += interp_spacing[name_x] / 2.0
            extent["ymin"] -= interp_spacing[name_y] / 2.0
            extent["ymax"] += interp_spacing[name_y] / 2.0
            ...
            if pdf_2D.min() < 0:  # Ensure the PDF is non-negative
                raise ValueError(
                    "The 2D PDF {0} has a negative value!".format(double_name)
                )
            ax_i.imshow(
                pdf_2D,
                vmin=0,
            ...
            if self.raw_gridpts is not None:
                ...
                raw_gridpoints_iter = itertools.product(
                    self.raw_gridpts[name_x], self.raw_gridpts[name_y]
                )
            ...
            ax_i.scatter(
                ...
                linewidth=0,
                facecolor="maroon",
                label="Model defined by peaks of 1D PDFs",
            )
            # Show peak of 2D pdf:
            ...
            max_inds_nd = np.unravel_index(
                np.argmax(NB_nd_pdf.nd_pdf), NB_nd_pdf.nd_pdf.shape
            )
            ...
            if ind_x == n - 1:  # If we're in the first column of plots
                ...
                label_y = (
                    layout_spec["bottom"]
                    + ind_y * layout_spec["hspace"]
                    + (ind_y + 0.5) * layout_spec["axes_height"]
                )
                ax_i.annotate(
                    G.param_display_names[ind_y],
                    (label_x, label_y),
                    xycoords="figure fraction",
                    rotation="vertical",
                    **self.label_kwargs
        ...
        for ind, param in enumerate(G.param_names):
            ...
            ax_k.plot(
                par_arr_map[param], pdf_1D, color="black", linewidth=0.9, zorder=6
            )
            ...
            ax_k.plot(
                [p_estimates[G.param_names[ind]]] * 2,
                y_lim,
                lw=0.6,
                linestyle="--",
                dashes=(3, 1.4),
            ...
            ax_k.set_xlim(
                np.min(par_arr_map[param]) - interp_spacing[param] / 2.0,
                np.max(par_arr_map[param]) + interp_spacing[param] / 2.0,
            )
            ...
            ax_k.tick_params(
                direction="out", length=0.7 * self.tick_size, which="minor"
            )
        ...
        if config1["show_legend"] is True and n > 1:
            ...
            self._add_legend(
                ax_k, ax_i, layout_spec, fontsize=config1["legend_fontsize"]
            )
        ...
        if config1["table_on_plot"] is True:
            anno_location = (
                layout_spec["left"]
                + (n // 2 * (layout_spec["axes_width"] + layout_spec["wspace"]) + 0.01),
                layout_spec["top"],
            )
            if n == 1:
